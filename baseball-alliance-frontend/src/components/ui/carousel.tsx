// src/components/MobileScroller.tsx  (gap-free)
...
export default function MobileScroller({
  ...
  const cardBasis = `basis-[${cardPercent}%]`;

  return (
    // lock to parent width (no viewport math)
    <div className={`relative w-full max-w-full overflow-hidden ${className}`}>
      {/* fades (keep, but they don't affect layout) */}
      ...
      <div
        ...
        style={{
          ...
          // center the active card without adding any extra gutter
      ...
      >
        ...
        {items.map((it) => (
          <article
            key={it.id}
            className={`${cardBasis} min-w-0 shrink-0 snap-center`}
          >
