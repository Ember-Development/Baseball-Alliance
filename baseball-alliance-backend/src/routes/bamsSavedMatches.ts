...
function matchCount(matchResponse: unknown): number {
  if (
    matchResponse &&
    typeof matchResponse === "object" &&
    Array.isArray((matchResponse as { matches?: unknown[] }).matches)
  ) {
    return (matchResponse as { matches: unknown[] }).matches.length;
  }
  return 0;
}
...
r.post("/", async (req, res) => {
  ...
  const saved = await prisma.bamsSavedMatch.create({
    data: {
      ...
      label: data.label,
      matchResponse: data.matchResponse as Prisma.InputJsonValue,
...
r.get("/:id", async (req, res) => {
  ...
  if (!saved) return res.status(404).json({ error: "Saved match not found" });
