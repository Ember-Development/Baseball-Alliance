...
function normalizeWeights(p?: MatchRequestV1["priorities"]): Record<
  (typeof WEIGHT_KEYS)[number],
  number
> {
  const raw = WEIGHT_KEYS.map((k) =>
    p?.[k] === undefined ? 1 : Math.max(0, Math.min(1, p[k]!))
  );
...
function fitLabel(overall: number): string {
  if (overall >= 0.75) return "Strong match";
  if (overall >= 0.55) return "Good match";
  return "Fair match";
}
...
function scoreProgram(
  ...
  const divisionHit =
    req.preferredDivisions.length === 0 ||
    req.preferredDivisions.some(
      (d) => d.toLowerCase() === p.division.toLowerCase()
    );
  const confHit =
    req.preferredConferences.length === 0 ||
    req.preferredConferences.some(
      (c) => c.toLowerCase() === p.conference.toLowerCase()
    );
  const stateHit =
    req.preferredStates.length === 0 ||
...
export async function runCollegeMatch(
  req: MatchRequestV1,
  query: MatchQuery
): Promise<MatchResponseV1> {
  ...
  const scored = programs.map((p) => {
    ...
    const overallScore =
      weights.athleticFit * breakdown.athleticFit +
    ...
    const row: MatchRowV1 = {
      ...
      scoreBreakdown: {
        ...
        affordabilityFit:
          Math.round(breakdown.affordabilityFit * 1000) / 1000,
      },
