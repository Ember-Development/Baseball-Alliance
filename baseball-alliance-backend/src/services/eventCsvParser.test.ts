...
describe("extractEventDateFromName", () => {
  it("pulls date from parentheses in event name", () => {
    expect(
      extractEventDateFromName(
        "Baseball Alliance x Action Baseball Showcase (6/7/26)"
      )
    ).toBe("6/7/26");
  });
});
...
describe("parseSyncEventExportCsv", () => {
  it("parses showcase sync export with Event ID, Event Name, and Playbook ID", () => {
    ...
    expect(athletes[0].athleteUuid).toBe(
      "8a3c490e-054d-40df-8a71-556f556abd0d:212"
    );
    expect(athletes[0].eventName).toBe(
      "Baseball Alliance x Action Baseball Showcase (6/7/26)"
    );
  ...
  it("requires Event ID, Event Name, and ID columns", () => {
    const { athletes, fileErrors } = parseSyncEventExportCsv(
      "Name,Email\nBob,bob@test.com"
    );
