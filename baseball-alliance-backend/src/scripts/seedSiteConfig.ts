...
const DEFAULT_WHO_WE_ARE =
  "Baseball Alliance connects players, teams, and events across the country.";
...
async function main() {
  ...
  const whoWeAre =
    process.env.SITE_WHO_WE_ARE_INITIAL?.trim() || DEFAULT_WHO_WE_ARE;
  ...
  try {
    ...
    if (existing) {
      console.log(
        `SiteConfig already exists (id=${existing.id}). Nothing to do.`,
      );
      return;
    }
