...
describe("College match V1 (integration)", () => {
    ...
    describe("GET /api/programs", () => {
        ...
        it("search returns Rice subset when fixture exists", async () => {
            ...
            expect(res.status).toBe(200);
            const names = res.body.data.map((r) => r.schoolName.toLowerCase());
    ...
    describe("GET /api/programs/:id", () => {
        it("404 for unknown id", async () => {
            const res = await request(app).get("/api/programs/clxxxxxxxxxxxxxxxxxxxxxxxxx");
    ...
    describe("POST /api/match", () => {
        ...
        it("invalid body: missing primaryPosition → 400 + details", async () => {
            ...
            expect(res.status).toBe(400);
            expect(res.body.error).toBe("Validation failed");
            const paths = res.body.details.fieldErrors.flatMap((fe) => fe.path.join("."));
            expect(paths.some((p) => p.includes("primaryPosition"))).toBe(true);
        });
            ...
            try {
                ...
                if (backup.length) {
                    await prisma.collegeProgram.createMany({
                        data: backup.map(({ id: _id, createdAt: _c, updatedAt: _u, ...rest }) => rest),
                    });
                }
            }
        });
