...
function fullNameFrom(input) {
    const parts = [input.firstName?.trim(), input.lastName?.trim()].filter((p) => Boolean(p && p.length > 0));
    return parts.length > 0 ? parts.join(" ") : "Administrator";
}
function normalizeEmail(email) {
    return email.trim().toLowerCase();
}
function parsePassword(row) {
    ...
    try {
        for (const row of entries) {
            ...
            if (passwordPlain !== undefined && passwordPlain.length < 6) {
                console.error(`Password for ${email} must be at least 6 characters when provided`);
                process.exit(1);
            }
