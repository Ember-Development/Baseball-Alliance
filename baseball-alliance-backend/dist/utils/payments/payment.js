...
import { getMerchantAuth, APIContracts, APIControllers, getEndpoint, } from "../payments/anetClient.js";
const { ACCEPT_HOSTED_RETURN_URL, ACCEPT_HOSTED_CANCEL_URL, COMBINE_PRICE_CENTS, } = process.env;
export async function createAcceptHostedSessionForCombine(opts) {
    ...
    if (reg.userId !== opts.userId)
        throw new Error("Forbidden");
    if (reg.status === "PAID")
        throw new Error("Already paid");
    const amountCents = typeof opts.amountCents === "number"
        ? opts.amountCents
    ...
    txnReq.setTransactionType(APIContracts.TransactionTypeEnum.AUTHCAPTURETRANSACTION);
    ...
    const token = await new Promise((resolve, reject) => {
        ctrl.execute(() => {
            const apiRes = new APIContracts.GetHostedPaymentPageResponse(ctrl.getResponse());
            if (apiRes.getMessages().getResultCode() === APIContracts.MessageTypeEnum.OK) {
    ...
    return {
        ...
        url: (process.env.AUTHORIZE_ENV || "SANDBOX").toUpperCase() === "PRODUCTION"
